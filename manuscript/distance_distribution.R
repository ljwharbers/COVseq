...
cutsites_nlaIII = c(109, 509, 515, 596, 787, 903, 968, 974, 1228, 
                   ...
                   27967, 28230, 28451, 29239, 29506, 29540, 29841)

cutsites_mseI = c(2, 77, 133, 402, 795, 867, 1035, 1094, 1275, 1362, 1602, 1665, 1671, 1791,
...
distance_nlaIII = as.data.table(sapply(1:(length(cutsites_nlaIII)-1), function(x) {
  cutsites_nlaIII[x + 1] - cutsites_nlaIII[x]
}))

distance_mseI = as.data.table(sapply(1:(length(cutsites_mseI)-1), function(x) {
  cutsites_mseI[x + 1] - cutsites_mseI[x]
}))

distance_nlaIII[, enzyme := "NlaIII"]
distance_mseI[, enzyme := "MseI"]

distance = rbind(distance_nlaIII, distance_mseI)

plt = ggplot(distance, aes(x = V1, color = enzyme)) +
  geom_density(adjust = 1.1) +
  scale_x_continuous(breaks = c(100, 200, 500, 1000), expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_color_brewer(palette = "Set1") + 
  labs(x = "Distance between consecutive cutsites (bp)")

save_and_plot(plt, "/mnt/AchTeraD/Documents/Projects/COVID19/Plots/distance-distribution-NlaIII_MseI",
              height = 5, width = 7)
