...
libraries = list("10K-copies" = "/mnt/AchTeraD/data/BICRO251/TN10/trimmed/CATCATCC+TGATGGAA_merged.bam",
                 "5K-copies" = "/mnt/AchTeraD/data/BICRO251/TN10/trimmed/TCACACGC+ATGTGTCA_merged.bam")
...
save_and_plot(plt1, "/mnt/AchTeraD/Documents/Projects/COVseq/Plots/barplots-boc/TN10-BreadtOfCoverage-10K-5K-fullregion",
              height = 6, width = 8)

plt2 = ggplot(total_m[region == "S-region"], aes(x = depth, y = value, fill = sample)) +
  geom_col(position = position_dodge()) +
  scale_fill_viridis_d(end = 0.5) +
  geom_hline(yintercept = c(theoret$`NlaIII + MseI`[4]), color = "red", linetype = "dashed", size = 1) +
  labs(y = "Breadth of Coverage", x = "Sequencing depth")

save_and_plot(plt2, "/mnt/AchTeraD/Documents/Projects/COVseq/Plots/barplots-boc/TN10-BreadtOfCoverage-10K-5K-sregion",
              height = 6, width = 8)
