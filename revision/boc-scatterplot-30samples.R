...
dt = data.table("NEBNext" = total[library == "NEBNext"]$full_10x,
                "COVseq" = total[library == "MS147" & !grepl("Neg|Pos", sample)]$full_10x)

plt = ggplot(dt, aes(x = NEBNext, y = COVseq)) + 
  geom_point(size = 2) +
  scale_x_continuous(limits = c(0, 1)) +
  scale_y_continuous(limits = c(0, 1)) +
  geom_smooth(formula = y ~ x, method = "lm", se = F, color = "red", linetype = 1, fullrange = TRUE) +
  stat_cor() +
